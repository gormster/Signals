...
final public class Signal<T> {
    ...
    private func addSource(source: Signal<T>, subscribeFunction: (AnyObject, @escaping SignalCallback) -> SignalSubscription<T>) -> SignalSubscription<T> {
        self.sources.append(source)
        return subscribeFunction(self) { [weak self] in
            self?.fire($0)
        }
    }
    
    @discardableResult
    public func subscribe(source: Signal<T>) -> SignalSubscription<T> {
        return self.addSource(source: source, subscribeFunction: source.subscribe)
    }
    
    @discardableResult
    public func subscribePast(source: Signal<T>) -> SignalSubscription<T> {
        return self.addSource(source: source, subscribeFunction: source.subscribePast)
    }
    
    @discardableResult
    public func subscribeOnce(source: Signal<T>) -> SignalSubscription<T> {
        return self.addSource(source: source, subscribeFunction: source.subscribeOnce)
    }
    
    @discardableResult
    public func subscribePastOnce(source: Signal<T>) -> SignalSubscription<T> {
        return self.addSource(source: source, subscribeFunction: source.subscribePastOnce)
    }
